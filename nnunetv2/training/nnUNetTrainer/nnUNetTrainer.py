...
class Attention_block(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.size()
        fc1 = self.fc(x).view([h, w, c, b])  # b,c->b,c//ratio->b,c->b,c,1,1
        fc1 = fc1.view([b, c, h, w])

        fc2 = self.fc(x).view([w, h, c, b])  # b,c->b,c//ratio->b,c->b,c,1,1
        fc2 = fc2.view([b, c, h, w])

        fc3 = self.fc(x).view([w, c, h, b])  # b,c->b,c//ratio->b,c->b,c,1,1
        fc3 = fc3.view([b, c, h, w])

        return [fc1, fc2,fc3]

class ThreConv(nn.Module):
    def __init__(self, units):
        super(ThreConv, self).__init__()
        self.units = units

    def forward(self,  inputs1,inputs2, inputs3,inputs4,b=0):

        fea1 = inputs1
        fea2= inputs2
        fea3= inputs3
        fea4= inputs4
        one = torch.ones_like(fea1)
        zero = torch.zeros_like(fea1)
        Thresholda = torch.where((fea2 + fea3 + fea4)/3 < 0.3, zero, one)

        Threshold_conv = torch.mul(fea1, Thresholda)

        return Threshold_conv
...
class TwoConv(nn.Module):
    ...
    def forward(self, inputs1,inputs2,inputs3,b=0):
        ...
        one = torch.ones_like(fea1)
        zero = torch.zeros_like(fea1)
        Thresholda = torch.where((fea2 + fea3)/2 < 0.3, zero, one)
        Threshold_conv = torch.mul(fea1, Thresholda)
        return Threshold_conv
...
class Ourmodel(nn.Module):
    def __init__(self,input_nbr=1,label_nbr=2):
        ...
        self.Attention_block_1 = Attention_block(256)
        self.ThreConv_1 = ThreConv(64)

        self.conv21 = nn.Conv2d(64, 128, kernel_size=3, padding=1)
        self.bn21 = nn.BatchNorm2d(128, momentum= batchNorm_momentum)
        self.conv22 = nn.Conv2d(128, 128, kernel_size=3, padding=1)
        self.bn22 = nn.BatchNorm2d(128, momentum= batchNorm_momentum)
        self.Attention_block_2 = Attention_block(128)
        self.ThreConv_2 = ThreConv(128)

        self.conv31 = nn.Conv2d(128, 256, kernel_size=3, padding=1)
        self.bn31 = nn.BatchNorm2d(256, momentum= batchNorm_momentum)
        self.conv32 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bn32 = nn.BatchNorm2d(256, momentum= batchNorm_momentum)
        self.Attention_block_3 = Attention_block(64)
        self.ThreConv_3 = ThreConv(256)

        self.conv41 = nn.Conv2d(256, 512, kernel_size=3, padding=1)
        self.bn41 = nn.BatchNorm2d(512, momentum= batchNorm_momentum)
        self.conv42 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.bn42 = nn.BatchNorm2d(512, momentum= batchNorm_momentum)
        self.Attention_block_4 = Attention_block(32)
        self.ThreConv_4 = ThreConv(512)
        ...
        self.Attention_block_5 = Attention_block(16)
        self.ThreConv_5 = ThreConv(512)



        self.conv53d = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.bn53d = nn.BatchNorm2d(512, momentum= batchNorm_momentum)
        self.conv52d = nn.Conv2d(512 *3, 512, kernel_size=3, padding=1)
        self.bn52d = nn.BatchNorm2d(512, momentum= batchNorm_momentum)
        self.conv51d = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.bn51d = nn.BatchNorm2d(512, momentum= batchNorm_momentum)

        self.Attention_block_6 = Attention_block(16)
        self.ThreConv_6 = TwoConv(512)


        self.conv43d = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.bn43d = nn.BatchNorm2d(512, momentum= batchNorm_momentum)
        self.conv42d = nn.Conv2d(512  *3, 512, kernel_size=3, padding=1)
        self.bn42d = nn.BatchNorm2d(512, momentum= batchNorm_momentum)
        self.conv41d = nn.Conv2d(512, 256, kernel_size=3, padding=1)
        self.bn41d = nn.BatchNorm2d(256, momentum= batchNorm_momentum)
        self.Attention_block_7 = Attention_block(32)
        self.ThreConv_7 = TwoConv(256)


        self.conv33d = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.bn33d = nn.BatchNorm2d(256, momentum= batchNorm_momentum)
        self.conv32d = nn.Conv2d(256  *3, 256, kernel_size=3, padding=1)
        self.bn32d = nn.BatchNorm2d(256, momentum= batchNorm_momentum)
        self.conv31d = nn.Conv2d(256,  128, kernel_size=3, padding=1)
        self.bn31d = nn.BatchNorm2d(128, momentum= batchNorm_momentum)
        self.Attention_block_8 = Attention_block(64)
        self.ThreConv_8 = TwoConv(128)

        self.conv23d = nn.Conv2d(128, 128, kernel_size=3, padding=1)
        self.bn23d = nn.BatchNorm2d(128, momentum=batchNorm_momentum)
        self.conv22d = nn.Conv2d(128 *3 , 128, kernel_size=3, padding=1)
        self.bn22d = nn.BatchNorm2d(128 , momentum= batchNorm_momentum)
        self.conv21d = nn.Conv2d(128, 64, kernel_size=3, padding=1)
        self.bn21d = nn.BatchNorm2d(64, momentum= batchNorm_momentum)
        self.Attention_block_9 = Attention_block(128)
        self.ThreConv_9 = TwoConv(64)

        self.conv12d = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        self.bn12d = nn.BatchNorm2d(64, momentum= batchNorm_momentum)
        self.conv11d = nn.Conv2d(64, label_nbr, kernel_size=3, padding=1)
        # self.Attention_block_10 = Attention_block()
        # self.ThreConv_10 = TwoConv()
    ...
    def forward(self, x):
        ...
        h1,v1,d1 = self.Attention_block_1(x12)

        x12_t = self.ThreConv_1(x12,h1,v1,d1)
        x1p, id1 = F.max_pool2d(x12_t,kernel_size=2, stride=2,return_indices=True)

        # Stage 2
        x21 = F.relu(self.bn21(self.conv21(x1p)))
        x22 = F.relu(self.bn22(self.conv22(x21)))
        h2, v2, d2 = self.Attention_block_2(x22)
        x22_t = self.ThreConv_2(x22, h2, v2, d2)
        x2p, id2 = F.max_pool2d(x22_t,kernel_size=2, stride=2,return_indices=True)

        # Stage 3
        x31 = F.relu(self.bn31(self.conv31(x2p)))
        x32 = F.relu(self.bn32(self.conv32(x31)))
        h3, v3, d3 = self.Attention_block_3(x32)
        x32_t = self.ThreConv_3(x32, h3, v3, d3)
        x3p, id3 = F.max_pool2d(x32_t,kernel_size=2, stride=2,return_indices=True)

        # Stage 4
        x41 = F.relu(self.bn41(self.conv41(x3p)))
        x42 = F.relu(self.bn42(self.conv42(x41)))
        h4, v4, d4 = self.Attention_block_4(x42)
        x42_t = self.ThreConv_4(x42,  h4, v4, d4)
        x4p, id4 = F.max_pool2d(x42_t,kernel_size=2, stride=2,return_indices=True)

        # Stage 5
        x51 = F.relu(self.bn51(self.conv51(x4p)))
        x52 = F.relu(self.bn52(self.conv52(x51)))
        h5, v5, d5 = self.Attention_block_5(x52)
        x52_t = self.ThreConv_5(x52, h5, v5, d5)
        x5p, id5 = F.max_pool2d(x52_t,kernel_size=2, stride=2,return_indices=True)


        # Stage 5d
        x5d = F.max_unpool2d(x5p, id5, kernel_size=2, stride=2)

        h6, v6, d6 = self.Attention_block_6(x5d)
        x61_t = self.ThreConv_6(x5d, h6, v6)
        x62_t = self.ThreConv_6(x5d, v6, d6)
        x63_t = self.ThreConv_6(x5d, h6, d6)
        x5d =  x61_t + x62_t + x63_t

        x53d = F.relu(self.bn53d(self.conv53d(x5d)))

        merge53 = torch.cat([x53d, x52,x52_t], dim=1)

        x52d = F.relu(self.bn52d(self.conv52d(merge53)))
        x51d = F.relu(self.bn51d(self.conv51d(x52d)))

        # Stage 4d
        x4d = F.max_unpool2d(x51d, id4, kernel_size=2, stride=2)
        h7, v7, d7 = self.Attention_block_7(x4d)
        x71_t = self.ThreConv_7(x4d, h7, v7)
        x72_t = self.ThreConv_7(x4d, v7, d7)
        x73_t = self.ThreConv_7(x4d, h7, d7)
        x4d = x71_t + x72_t + x73_t


        x43d = F.relu(self.bn43d(self.conv43d(x4d)))
        merge43 = torch.cat([x43d, x42, x42_t], dim=1)
        x42d = F.relu(self.bn42d(self.conv42d(merge43)))
        x41d = F.relu(self.bn41d(self.conv41d(x42d)))

        # Stage 3d
        x3d = F.max_unpool2d(x41d, id3, kernel_size=2, stride=2)
        h8, v8, d8 = self.Attention_block_8(x3d)
        x81_t = self.ThreConv_8(x3d, h8, v8)
        x82_t = self.ThreConv_8(x3d, v8, d8)
        x83_t = self.ThreConv_8(x3d, h8, d8)
        x3d = x81_t + x82_t + x83_t


        x33d = F.relu(self.bn33d(self.conv33d(x3d)))
        merge33 = torch.cat([x33d, x32, x32_t], dim=1)

        x32d = F.relu(self.bn32d(self.conv32d(merge33)))
        x31d = F.relu(self.bn31d(self.conv31d(x32d)))

        # Stage 2d
        x2d = F.max_unpool2d(x31d, id2, kernel_size=2, stride=2)
        h9, v9, d9 = self.Attention_block_9(x2d)
        x91_t = self.ThreConv_9(x2d, h9, v9)
        x92_t = self.ThreConv_9(x2d, v9, d9)
        x93_t = self.ThreConv_9(x2d, h9, d9)
        x2d = x91_t + x92_t + x93_t


        x23d = F.relu(self.bn22d(self.conv23d(x2d)))
        merge23 = torch.cat([x23d, x22, x22_t], dim=1)
        x22d = F.relu(self.bn22d(self.conv22d(merge23)))
    ...
    def initialize(self):
        if not self.was_initialized:
            self.num_input_channels = determine_num_input_channels(self.plans_manager, self.configuration_manager,
                                                                   self.dataset_json)
            self.network=Ourmodel().to(self.device)
